...
function CheckQuest() 
    ...
    if World1 then
        ...
        elseif MyLevel == 1725 or MyLevel <= 1199 or SelectMonter ==  "Lava Pirate" then
            Mon = "Lava Pirate"
...
local Tabs = {
	...
	General3 = Window:AddTab({ Title = "Visuals ", Icon = "scroll" }),
...
local function autoClick()
    while isAutoClickActive do
        VirtualInputManager:SendMouseButtonEvent(Vector2.new(1280, 672))
        task.wait(0.1)
        VirtualInputManager:SendMouseButtonEvent(Vector2.new(1280, 672))
        task.wait(0.05)
    end
				...
				pcall(function()
					if game.Players.LocalPlayer.Data.Level.Value >= 700 and World1 then
						...
						if game.Workspace.Map.Ice.Door.CanCollide == true and game.Workspace.Map.Ice.Door.Transparency == 0 then
							repeat wait() TP(CFrame.new(4851.8720703125, 5.6514348983765, 718.47094726563)) until (CFrame.new(4851.8720703125, 5.6514348983765, 718.47094726563).Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoSecondSea
							wait(1)
							game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("DressrosaQuestProgress","Detective")
							EquipWeapon("Key")
							local Pos = CFrame.new(1347.7124, 37.3751602, -1325.6488)
							repeat wait() TP(Pos) until (Pos.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoSecondSea
							wait(3)
...
-- ฟังก์ชันสำหรับจัดการศัตรู
local function HandleEnemy(v, weapon, magnetCFrame)
    if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
        repeat
            task.wait()
            AutoHaki() -- เปิดใช้งาน Haki
            EquipWeapon(weapon) -- ใช้อาวุธที่เลือก
            v.HumanoidRootPart.CanCollide = false
            v.Humanoid.WalkSpeed = 0
            v.Head.CanCollide = false
            StartMagnetBoneMon = true
            PosMonBone = v.HumanoidRootPart.CFrame
            topos(v.HumanoidRootPart.CFrame * magnetCFrame)
            game:GetService("VirtualUser"):CaptureController()
            game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
...
-- การจัดการศัตรูใน ReplicatedStorage
local function HandleReplicatedEnemies()
...
-- โค้ดหลักสำหรับการจัดการ Auto Bone
spawn(function()
    while task.wait() do
        if _G.Auto_Bone and World3 then
            pcall(function()
                local enemies = game:GetService("Workspace").Enemies
                if enemies:FindFirstChild("Reborn Skeleton") or 
                   enemies:FindFirstChild("Living Zombie") or 
                   enemies:FindFirstChild("Demonic Soul") or 
                   enemies:FindFirstChild("Posessed Mummy") then
                    
                    for _, v in pairs(enemies:GetChildren()) do
                        if v.Name == "Reborn Skeleton" or v.Name == "Living Zombie" or 
                           v.Name == "Demonic Soul" or v.Name == "Posessed Mummy" then
                            HandleEnemy(v, _G.SelectWeapon, CFrame.new(0, 25, 15))
                        end
                    end
                else
                    topos(CFrame.new(-9466.72949, 171.162918, 6132.01514)) -- ตำแหน่งสำรองเมื่อไม่มีศัตรู
                    StartMagnetBoneMon = false
                    HandleReplicatedEnemies()
                end
            end)
        end
    end
...
-- การตั้งค่าเพิ่มเติมสำหรับการจัดการศัตรู
spawn(function()
    while task.wait() do
        for _, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
            if v.Name == "Reborn Skeleton" or v.Name == "Living Zombie" or 
               v.Name == "Demonic Soul" or v.Name == "Posessed Mummy" then
                for _, y in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if y.Name == "Reborn Skeleton" or y.Name == "Living Zombie" or 
                       y.Name == "Demonic Soul" or y.Name == "Posessed Mummy" then
                        -- จัดการการเคลื่อนไหวของศัตรู
...
local General3 = Tabs.General3:AddSection("Sword")
local Toggle = Tabs.General3:AddToggle("AutoSaber", {
    Title = "Auto Saber", 
...
local Slider = Tabs.Settings:AddSlider("Fps", 
{
    ...
    Rounding = 1,
    Callback = function(Value)
        _G.setfpscap = Value -- กำหนดค่า FPS ตาม Slider
    end
})

-- สร้าง Toggle สำหรับเปิด/ปิดการล็อค FPS
...
-- กระบวนการควบคุม FPS
task.spawn(function()
    while task.wait() do -- ใช้ task.wait() เพื่อรอบการทำงาน
        pcall(function()
            if _G.lockFps then -- ตรวจสอบว่าการล็อค FPS เปิดอยู่หรือไม่
                if type(_G.setfpscap) == "number" then -- ตรวจสอบว่า _G.setfpscap เป็นตัวเลข
                    setfpscap(_G.setfpscap) -- เรียกใช้ฟังก์ชัน setfpscap
                end
            else
                setfpscap(0) -- ปิดการล็อค FPS (0 = ไม่จำกัด)
            end
        end)
    end
end)
